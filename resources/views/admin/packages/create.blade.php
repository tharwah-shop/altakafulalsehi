...
@section('title', isset($package) ? 'تعديل الباقة' : 'إضافة باقة جديدة')

@section('content')
<!-- Header -->
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="fw-bold mb-1">{{ isset($package) ? 'تعديل الباقة' : 'إضافة باقة جديدة' }}</h2>
        <p class="text-muted mb-0">{{ isset($package) ? 'تحديث بيانات الباقة' : 'إنشاء باقة اشتراك جديدة' }}</p>
    </div>
    <div>
        ...
        <div class="card card-flush h-xl-100">
            <div class="card-header pt-7">
                <h3 class="card-title align-items-start flex-column">
                    ...
                    <span class="text-gray-400 mt-1 fw-semibold fs-6">أدخل البيانات الأساسية للباقة</span>
                </h3>
            </div>
            <div class="card-body pt-6">
                <form action="{{ isset($package) ? route('admin.packages.update', $package->id) : route('admin.packages.store') }}" method="POST" id="packageForm">
                    @csrf
                    @if(isset($package))
                        @method('PUT')
                    @endif
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group">
                                <input type="number" name="discount_percentage" class="form-control @error('discount_percentage') is-invalid @enderror"
                                       value="{{ old('discount_percentage', $package->discount_percentage ?? 0) }}" placeholder="30" step="0.01" min="0" max="100">
                    ...
                    <div class="row mb-6">
                        <div class="col-md-4">
                            ...
                            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                <option value="active" {{ old('status', $package->status ?? 'active') === 'active' ? 'selected' : '' }}>نشط</option>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="sort_order" class="form-control @error('sort_order') is-invalid @enderror"
                                   value="{{ old('sort_order', $package->sort_order ?? 0) }}" placeholder="0" min="0">
                            @error('sort_order')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="color" name="color" class="form-control form-control-color @error('color') is-invalid @enderror"
                                   value="{{ old('color', $package->color ?? '#007bff') }}">
                            @error('color')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="card-body pt-6">
                <div class="card border" id="package-preview">
                    <div class="card-header text-center" style="background-color: #007bff; color: white;">
                        <h5 class="mb-0" id="preview-name">اسم الباقة</h5>
                        <small id="preview-name-en">Package Name</small>
                    </div>
                    <div class="card-body text-center">
                        <div class="display-6 fw-bold text-primary mb-2" id="preview-price">0 ريال</div>
                        <div class="text-muted mb-3" id="preview-duration">مدة الاشتراك</div>
                        <div class="text-muted mb-3" id="preview-dependents">عدد التابعين</div>
                        <ul class="list-unstyled text-start" id="preview-features">
                            <li><i class="fas fa-check text-success me-2"></i>ميزة 1</li>
                        </ul>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        ...
        const dependentPrice = document.querySelector('[name="dependent_price"]').value;
